...
| Spend caps and reset audit trail | PASS (source/CI); BLOCKED (packaged mutation) | Atomic budget, daily/monthly caps, reset adjustments, crash recovery, and authorization suites pass in CI. | Daily cap 10→9 succeeded; monthly cap 150→149 failed with missing `X-Tether-Response-Signature`; diagnose issues #14/#15 and retest both values plus restart persistence. |
