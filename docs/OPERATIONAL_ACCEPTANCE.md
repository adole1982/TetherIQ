...
| Bedrock Claude path | PASS | Run-42 packaged app authenticated a Bedrock route and Claude Code returned exact `TETHERMESH_OK`. | Repeat once against the run-45 package. |
...
| Clean packaged Windows install | PASS (launch) | Release run 45 (`34505807778`) passed all four platform jobs. Windows artifact `10164281193` was downloaded, extracted, installed into `C:\Users\alexd\AppData\Local\Temp\tethermesh-run45-installed`, and `tethermesh.exe` launched successfully. | Complete final gateway, Bedrock, restart, and stream checks on this exact install. |
