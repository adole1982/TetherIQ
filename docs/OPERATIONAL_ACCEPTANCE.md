...
| Gateway readiness and authenticated model routes | BLOCKED (visible runtime) | Native `test_gateway_route` checks readiness and authenticated `/v1/models`; packaged run 42 returned one authenticated model route on a dynamic loopback port. Release run 45 published the final Windows package for the focused runtime check. | Visible final-package gateway check remains open. |
| Bedrock Claude path | BLOCKED (final-package runtime) | Run-42 packaged app authenticated a Bedrock route and Claude Code returned exact `TETHERMESH_OK`. | Repeat once against the run-45 package. |
...
| Telemetry, redaction, and SSE | PASS (source/CI); BLOCKED (live observation) | Secret-scrubbing, telemetry authorization, structured traces, stream accounting, and SSE implementation are covered by security suites. | Observe one live stream and telemetry event in the final packaged run. |
| Sidecar lifecycle and restart recovery | PASS (source/CI); BLOCKED (final-package restart) | Lifecycle and crash-recovery suites pass; route-sync retry fixes saved-credential restart behavior. | Repeat close/reopen and route recovery on the final package. |
