...
export const ActiveAgentTracker: React.FC = () => {
  const { connectedAgents, budget } = useTetherStore();
      ...
      <div className="p-6 rounded-xl bg-gradient-to-r from-slate-900 via-slate-900/90 to-slate-950 border border-slate-800 flex items-center justify-between">
        <div className="space-y-1 max-w-xl">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xs font-bold uppercase tracking-wider text-cyan-400">Agent Session Management</span>
          </div>
          <h1 className="text-xl font-extrabold text-white tracking-tight">Active Connected Agents</h1>
          <p className="text-xs text-slate-400 leading-relaxed">
            Monitor all live coding agents connected to <code className="text-cyan-300 font-mono">127.0.0.1:4000</code>. View real-time token usage, per-session costs, active model paths, and connection IPs.
          </p>
        </div>
        ...
        <div className="flex items-center space-x-2 text-xs font-mono bg-slate-950 px-3 py-2 rounded-lg border border-slate-800">
          ...
          <span className="text-slate-300">{connectedAgents.length} Active Loopback Sessions</span>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900/70 overflow-hidden shadow-sm">
        ...
        <div className="divide-y divide-slate-800/60">
          {connectedAgents.map((agent) => {
            const connectedMinutes = Math.round((Date.now() - agent.connectedAt) / 60000);
            return (
              <div
                key={agent.id}
